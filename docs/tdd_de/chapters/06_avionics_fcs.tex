...
\label{sec:pinout_port_assignment_matrix}

Zur systematischen Dokumentation und Vermeidung von Fehlverkabelungen innerhalb der Hardware-Integration fasst Tabelle~\ref{tab:pixhawk_port_assignment} die physische Belegungsmatrix des \textit{Pixhawk 6X v2B Standard Baseboards} zusammen. Die Schnittstellen sind strikt nach ihrer funktionalen Priorität und den zugrundeliegenden Bus-Topologien (Serial UART, CAN-Bus, Ethernet, PWM und Power) partitioniert.

\begin{table}[h!]
    \centering
    \caption{Physische Port-Belegungsmatrix des Pixhawk 6X v2B Baseboards}
    \vspace{0.2cm}
    \begin{small}
    \begin{tabular}{p{2.5cm} p{4.5cm} p{3.2cm} p{2.8cm}}
        \toprule
        \textbf{Baseboard Port} & \textbf{Angeschlossenes Subsystem} & \textbf{Protokoll / Signal} & \textbf{Versorgung} \\
        \midrule
        \texttt{POWER1} & Mauch 053 Power-Cube 3 (Primär) & Analog / Telemetrie & $\SI{5.3}{V}$ System IN \\
        \texttt{POWER2} & Mauch 053 Power-Cube 3 (Sekundär) & Analog / Telemetrie & $\SI{5.3}{V}$ System IN (Redundanz) \\
        \midrule
        \texttt{GPS1} & Holybro M10 GNSS & UBX / MAVLink / I2C & $\SI{5.0}{V}$ via Port \\
        \texttt{GPS2} & Holybro H-RTK F9P Rover GPS & UBX / RTCM3 & $\SI{5.0}{V}$ via Port \\
        \midrule
        \texttt{TELEM1} & CubePilot Herelink Air Unit v1.1 & MAVLink Telemetrie & $\SI{5.0}{V}$ via Port \\
        \texttt{TELEM2} & uAvionix pingRX Pro (ADS-B In) & MAVLink (DAA) & $\SI{5.0}{V}$ via Port \\
        \texttt{TELEM3} & LightWare SF11/C LiDAR & Serial / UART (AGL) & $\SI{5.0}{V}$ via Port \\
        \midrule
        \texttt{CAN1} & CAN-Bus-Strang (DLVR Airspeed, RM3100 Compass, ARK Flow, Remote ID) & DroneCAN / Cyphal ($1\,Mbps$) & $\SI{5.0}{V}$ via Bus \\
        \texttt{CAN2} & Reserved / Expansion Port & -- & -- \\
        \midrule
        \texttt{ETH} & NVIDIA Jetson Orin Nano / Network & MAVLink over UDP & Gigabit Ethernet \\
        \texttt{AD\&I/O} & NVIDIA Jetson Orin Nano Header & Serial / UART (Backup-Bridge) & Logic Level ($\SI{3.3}{V}$) \\
        \midrule
        \texttt{M1 -- M3} & Dronavia Zephyr CC ESC MF (FTS) $\rightarrow$ ESCs / Servos & PWM Steuerleitungen & Galv. Isoliert \\
        \bottomrule
    \end{tabular}
    \end{small}
    \label{tab:pixhawk_port_assignment}
\end{table}

Wie in Abbildung~\ref{fig:avionics_architecture} dargestellt, sichert diese Topologie eine lückenlose Trennung zwischen der hochfrequenten Echtzeit-Regelung auf den CAN- und PWM-Bussen sowie dem datenintensiven Netzwerkverkehr über den \texttt{ETH}-Port ab.
...
\label{sec:bus_topologies}

Die vernetzte Systemarchitektur des UAS basiert auf einer strikten Trennung zwischen deterministischen Echtzeit-Steuerungsdaten und hochbandbreitigen Missionsdatenströmen (siehe Systemübersicht in Abb.~\ref{fig:avionics_architecture}).

\subsubsection{Deterministic Control via CAN-Bus (UAVCAN / Cyphal)}
%Für die flugsicherheitskritische Peripheriekommunikation wird das differenzielle \textit{DroneCAN}-Protokoll (auf Basis von CAN-Bus mit $\SI{1}{\Mbps}$) genutzt. Diese Netzwerktopologie verbindet den primären Flugregler (Pixhawk 6X) in Reihe mit dem digitalen DLVR-Airspeed-Sensor, dem externen RM3100-Backup-Magnetometer und dem Holybro Remote ID Modul. Durch die physikalische Robustheit der CAN-Bus-Signalpegel wird eine deterministische, fehlertolerante Datenübertragung gewährleistet, die inhärent resistent gegen elektromagnetische Einkopplungen der Antriebsstränge ist.

\subsubsection{High-Bandwidth Inter-Processor Bridge (Ethernet)}
Die Übertragung von massiven Datenströmen – primär die H.264/H.265-RTSP-Videostreams der EO/IR-Gimbal-Kamera sowie der inter-prozessuale ROS~2-Netzwerkverkehr (via \textit{micro-XRCE-DDS}) zwischen der FMU und dem \textit{NVIDIA Jetson Orin Nano} – erfolgt über ein lokales Gigabit-Ethernet-Netzwerk (100BASE-TX). Die physikalische Verbindung wird an Bord über geschirmte, gewichtsoptimierte Cat.6-Flachbandkabel realisiert, um minimale Latenzen bei der KI-gestützten Personendetektion zu garantieren.

\subsection{Power Distribution Architecture \texorpdfstring{\&}{\&} Safety Isolation}
\label{sec:power_distribution}

Die elektrische Versorgung der Avionik ist konsequent redundant und galvanisch von den transienten Spannungsspitzen des Antriebssystems isoliert.

\subsubsection{Redundant Avionics Power Supply (BECs)}
Die redundante Spannungsversorgung der flugsicherheitskritischen Avionik-Schiene wird über ein dediziertes High-End-Stromversorgungssystem vom Typ \textit{Mauch 053 Power-Cube 3 (V3)} realisiert \cite{premium_mauch_053}. Dieses Modul ist für Eingangsspannungen von bis zu $\SI{60.0}{\V}$ ($14\text{S Applikationen}$) spezifiziert und transformiert die $\SI{44.4}{\V}$ Nennspannung des zentralen LiPo-Antriebsstrangs abwärts. Der Power-Cube stellt zwei galvanisch getrennte Ausgänge mit jeweils $\SI{5.3}{\V}$ bei einem kontinuierlichen Nennstrom von je $\SI{10}{\A}$ zur Verfügung. Diese werden über die mitgelieferten, geschirmten \textit{Clik-Mate-6p}-Kabelstränge redundant an die Primär- und Sekundärbuchsen (\texttt{POWER1} und \texttt{POWER2}) des \textit{Pixhawk 6X v2B Standard Baseboards} geführt. Ein integriertes Dioden-Netzwerk garantiert ein unterbrechungsfreies Hard-Failover im Mikrosekundenbereich. Zudem stellt das Modul einen autarken $\SI{12.0}{\V}$-Leistungskanal ($\SI{10}{\A}$ konstant) bereit, welcher exklusiv für die Versorgung des EO/IR-Gimbals und der Failsafe-Aktuatorik genutzt wird.

Zur vollständigen elektrischen Entkopplung des rechenintensiven Missionscomputers wird ein separater, ultrakompakter Abwärtsregler vom Typ \textit{Matek Systems mBEC12S} in das Avionik-Deck integriert \cite{premium_matek_mbec12s}. Das $\SI{2}{\g}$ leichte Modul operiert ebenfalls in einem Weitbereichs-Eingangsfeld bis $\SI{60}{\V}$ und wird über eine hardwarespezifische Lötbrücke fest auf eine Ausgangsspannung von $\SI{12.0}{\V}$ konfiguriert. Bei einer nominalen Systemspannung von $\SI{44.4}{\V}$ liefert das Modul einen kontinuierlichen Laststrom von über $\SI{1.3}{\A}$ sowie temporäre Stromspitzen von bis zu $\SI{3.0}{\A}$ ($\approx \SI{36}{\W}$ Peak). Dieser Ausgang speist direkt den DC-Barrel-Jack des \textit{NVIDIA Jetson Orin Nano Carrier Boards}. Durch diese strikte physische Partitionierung wird wirksam unterbunden, dass hochfrequente Lastwechsel oder transiente Störungen des KI-Missionscomputers auf die Messspannung der flugkritischen IMU-Sensorik durchschlagen.

\subsubsection{Galvanic Isolation of the Propulsion System}
Um zu verhindern, dass die massiven Stromänderungen ($\ge \SI{120}{\A}$) und die hochfrequente Taktung der drei Brushless-Hauptmotoren im VTOL-Startbetrieb auf die empfindliche Messsensorik (IMUs, Barometer) durchschlagen, ist der Antriebsstrang galvanisch vom Avionik-Regelkreis isoliert. Die Signalübertragung zu den ESCs und den Tri-Tilt-Servomotoren erfolgt über Optokoppler-Schnittstellen, was eine physische Trennung der Masse-Potentiale sicherstellt.

\subsection{Electromagnetic Compatibility (EMV) \texorpdfstring{\&}{\&} Shielding Concept}
\label{sec:emv_shielding}

Zur Gewährleistung der elektromagnetischen Verträglichkeit innerhalb der engen Struktur des Rumpfmonocoques wurden konstruktive Schutzmaßnahmen implementiert.

\subsubsection{Component Placement and Spatial Separation}
Die Systemkomponenten sind geometrisch nach ihrem Störpotenzial partitioniert. Während die Hochstrom-Leistungsverteilung (\textit{Power Distribution Board}) und die Motorregler im unteren Rumpfbereich positioniert sind, befindet sich das Avionik-Deck im geschützten oberen Segment. Sensoren mit hoher HF-Empfindlichkeit, wie das RM3100-Backup-Magnetometer und die GNSS-Empfänger, sind geometrisch isoliert in den Tragflächenspitzen bzw. auf einem CFK-Ausleger platziert.

\subsubsection{Physical Shielding and Grounding}
Signal- und Datenleitungen (insbesondere Ethernet- und Kamera-Signalkabel) sind mit einer hochflexiblen, ultraleichten EMV-Abschirmung versehen. Das gesamte UAS nutzt ein zentrales, sternförmiges Erdungskonzept (\textit{Single-Point Grounding}), bei dem alle Abschirmungen und Gehäusemassen an einem definierten Potenzialpunkt zusammenlaufen. Dies unterbindet das Entstehen von Masseschleifen, welche die Extended-Kalman-Filter-Berechnungen stören könnten.


\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{../data/06_avionics/Avionics_Sketch_Image.png}
    \caption{Systemtopologie-Sketch der Avionik-Infrastruktur}
...
Die exakte Erfassung aller Einzelmassen innerhalb des Avionik-, Sensorik- und Nutzlast-Stacks ist eine fundamentale Voraussetzung für die Bestimmung des Gesamtschwerpunkts (\textit{Center of Gravity} -- CoG) sowie für die aerodynamische Trimmung des Tri-Tilt-Rotor-UAS. Da das System ein modulares Payload-Concepts für die Suchkamera verfolgt, bildet die Massenbilanz in Tabelle~\ref{tab:avionics_mass_balance} das maximale Gewichtsszenario (unter Verwendung des schwersten \textit{Foxtech Seeker AI}-Gimbals inklusive mechanischer Dämpfungsadapter) ab.

\vspace{0.2cm}

\begin{longtable}{p{6.0cm} p{5.0cm} p{2.5cm}}
    \caption{Vollständige Massenbilanz des Avionik-, Sensorik- und Peripherie-Stacks} \label{tab:avionics_mass_balance} \\
    \toprule
    \textbf{Baugruppe / Komponente} & \textbf{Spezifikation / Hardware-Typ} & \textbf{Masse (g)} \\
    \midrule
    \endhead % Dieser Tabellenkopf wird auf jeder neuen Seite wiederholt
    
    \bottomrule
    \endfoot % Fußzeile für umgebrochene Seiten
    ...
    ~~LightWare SF11/C & Einelektronischer LiDAR-Entfernungsmesser & $\SI{35}{\g}$ \\
    ...
    \textbf{Spannungsversorgung (Power Stacks)} & & \\
    ~~Mauch 053 Power-Cube 3 (V3) & Redundantes 12S-Avionik-BEC inkl. Lüfter & $\SI{86}{\g}$ \\
    ~~Matek Systems mBEC12S & Separates 12S-Jetson-Spannungsmodul & $\SI{2}{\g}$ \\
    \midrule
    ...
    \textbf{Gesamtgewicht Avionik/Payload} & \textbf{Reine Elektronik- und Nutzlastmasse} & \textbf{\SI{1736.3}{\g} (\SI{1.736}{\kg})} \\
\end{longtable}
